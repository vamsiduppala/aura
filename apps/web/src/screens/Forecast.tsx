...
import type { Chart, Energy, ForecastPeriod, LifeArea } from '@aura/engine';
import { buildForecast, buildCustomForecast, generateExpandedReading } from '@aura/engine';
...
const FX_HEAD: Record<Energy, string> = {
  build: 'The heaviness has an exit.',
  crave: 'The noise settles into a shape.',
  main: 'Your visible season, mapped.',
  feel: 'The tides, and when they turn.',
  fire: 'Where the heat is heading.',
  mind: 'The next moves, in order.',
  grow: 'The open road ahead.',
  love: 'Where the warmth is heading.',
...
export function Forecast({ chart, now, chartSeed, goalArea, major }: {
  chart: Chart; now: Date; chartSeed: number; goalArea?: LifeArea; major: Energy;
}) {
  ...
  const fc = useMemo(() => buildForecast(chart, now), [chart, now]);
  const custom = useMemo(
    () => buildCustomForecast(chart, new Date(from), new Date(to), now),
    [chart, from, to, now],
  );
  ...
  return (
    <>
      <StatusBar />
      <Wordmark right={<span>forecast</span>} />
      <div className="view" style={{ overflow: 'hidden' }}>
        ...
        {detail ? (
          <DetailOverlay
            p={detail} major={major} chartSeed={chartSeed} goalArea={goalArea} tab={tab}
...
function DetailOverlay({ p, major, chartSeed, goalArea, tab, onClose }: {
  p: ForecastPeriod; major: Energy; chartSeed: number; goalArea?: LifeArea; tab: Tab; onClose: () => void;
}) {
  const r = generateExpandedReading(p.energy, major, p.start, p.end, chartSeed, goalArea ? { goalArea } : {});
  const c = energyColor(p.energy);
